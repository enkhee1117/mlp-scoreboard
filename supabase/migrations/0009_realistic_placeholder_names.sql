-- Replace generic "Player 1 … Player N" placeholder names with a shuffled
-- selection from a realistic name list so new tournaments look less like a
-- test fixture.
...
begin
  ...
  -- Seed placeholder players with shuffled realistic names.
  -- Uses modulo so it wraps safely if v_count > array_length(v_names).
  if v_count > 0 then
